...
Module.prototype._runScan = function(scope, scanOptions, callback) {
  ...
  var postScan = function(callback) {
    log.debug("Running postScan cleanup");
    return miouzikdb.cleanupMultipleDiscogsArtistsIDs(db, adminContext, function(err) {
      return callback(err);
    });
  }

  var endScan = function(err, cumulatedStats) {
    return postScan(function(err2) {
      if (err2)
        log.error(undefined, "Failed to executed postScan cleanup", err2);
      if (err) {
