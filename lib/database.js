...
loadSong = function(db, userContext, songId, callback) {
  ...
  return db.withConnection(function(client, callback) {    
    ...
    var query = "SELECT m.id, m.tagMTime, m.title, m.artist, m.album, m.year, m.trackNumber, m.genre, m.durationInSeconds, m.likes, m.playCount, " +
                "       m.discogsReleaseId AS rid, r.release AS rrelease, " +
                "       m.discogsArtistId AS aid, a.artist AS aartist, " +
                "       f.shortFilename, f.longFilename, f.mtime, f.size, f.md5, f.vanishedAt, f.hidden " +
                " FROM miouzik_songs m " +
                " LEFT JOIN photos_fingerprints f ON m.id = f.id " +
                " LEFT OUTER JOIN miouzik_discogs_releases r ON m.discogsReleaseId = r.id" +
...
/**
 * Cleanup
 * Sometimes, after renaming / changing files, it may happen that we have an artist with several different discogs ID
 * This cleanup procedure will only keep the first one
 * @param db - is the database
 * @param userContext - the user context for database access
 */
cleanupMultipleDiscogsArtistsIDs = function(db, userContext, callback) {
  if (!userContext)
    return callback(db.requiresRights("cleanupMultipleDiscogsArtistsIDs requires a user context"));

  return db.withConnection(function(client, callback) {
    // Get artists that have more than 1 discogsArtist ID
    var query = "SELECT DISTINCT m.artist, m.discogsArtistId FROM miouzik_songs m"
              + " WHERE m.artist IN ("
              + "   SELECT DISTINCT m.artist"
              + "   FROM miouzik_songs m "
              + "   LEFT JOIN photos_fingerprints f ON (f.id = m.id) "
              //+ "   WHERE f.longFileName LIKE '/Users/alex/Music/Alex%'"
              + "   GROUP BY m.artist"
              + "   HAVING count( distinct m.discogsArtistId) > 1"
              + "   ORDER BY m.artist"
              + " )"
              + " ORDER BY m.artist, m.discogsArtistId"
    var bindings = [ ];
    return db.query(client, "cleanupMultipleDiscogsArtistsIDs", query, bindings, function(err, result) {
      if (err) return callback(err);
      var artist = "";
      var discogsArtistId = undefined;
      var updates = [];
      debugger;
      for (var i=0; i<result.length; i++) {
        var row = result[i];
        var newArtist = row["artist"];
        var newDiscogsArtistId = +row["discogsartistid"]
        if (discogsArtistId === undefined) {
          artist = newArtist;
          discogsArtistId = newDiscogsArtistId;
          continue;
        }
        if (newArtist !== artist) {
          artist = newArtist;
        ...
        var update = "UPDATE miouzik_songs SET discogsArtistId=" + discogsArtistId + " WHERE discogsArtistId=" + newDiscogsArtistId + " AND artist='" + utils.escapeForWhere(newArtist) + "'";
        updates.push(update);
      }
      return db.executeSQL(userContext, updates, function(err) {
        if (err) return callback(err);
        return callback();
      });
    });
...
module.exports = {
  ...
  cleanupMultipleDiscogsArtistsIDs: cleanupMultipleDiscogsArtistsIDs
};
